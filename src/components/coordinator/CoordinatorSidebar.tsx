...
import { LayoutDashboard, Users, BookOpen, Settings, LogOut, Menu, X, Shield, FileText, Bot, GraduationCap, Layers3, BookCopy, Sparkles } from 'lucide-react'
...
function SidebarContent({
  ...
  return (
    <div className="relative flex h-full flex-col overflow-hidden bg-surface text-text-primary transition-colors duration-300">
      ...
      <div className="pointer-events-none absolute -top-16 -left-16 h-48 w-48 rounded-full bg-emerald-500/15 blur-3xl dark:bg-emerald-600/20 html.mid:bg-rose-500/20" />
      <div className="pointer-events-none absolute top-1/2 -right-16 h-48 w-48 rounded-full bg-teal-500/15 blur-3xl dark:bg-teal-600/15 html.mid:bg-amber-500/15" />

      {/* Sidebar Header / Logo */}
      <div className="relative z-10 flex h-20 items-center justify-between border-b border-border px-5 backdrop-blur-md">
        <Link href="/coordinator/dashboard" onClick={onClose} className="flex items-center gap-3 group">
          <div className="relative flex h-10 w-10 items-center justify-center rounded-2xl bg-gradient-to-br from-emerald-400 via-emerald-600 to-teal-600 p-0.5 shadow-lg shadow-emerald-500/25 group-hover:scale-105 transition-transform duration-200">
          ...
          <div>
            ...
            <small className="block text-[10px] font-extrabold uppercase tracking-[0.16em] text-emerald-600 dark:text-emerald-400 html.mid:text-rose-400">Coordinator Portal</small>
          </div>
        </Link>
        {isMobile && (
          <button
            ...
            >
              {isActive && (
                <motion.div
                  ...
                  className="absolute inset-0 rounded-2xl bg-gradient-to-r from-emerald-600 via-emerald-500 to-teal-600 shadow-lg shadow-emerald-500/30"
      ...
      <div className="relative z-10 border-t border-border p-3.5 backdrop-blur-md">
        <div className="rounded-2xl border border-border bg-surface-hover/60 p-3 shadow-sm backdrop-blur-xl transition-all duration-200 hover:border-primary/40">
          <div className="flex items-center gap-3">
            <div className="relative flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-emerald-400 to-teal-500 p-0.5 font-bold text-white shadow-md">
              <div className="flex h-full w-full items-center justify-center rounded-[10px] bg-surface font-black text-xs text-emerald-600 dark:text-emerald-300">
                <Shield className="h-4 w-4" />
              </div>
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-xs font-bold text-text-primary truncate">{user?.full_name || 'Coordinator'}</p>
              <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-emerald-600 dark:text-emerald-400 html.mid:text-rose-400 truncate">
                <Sparkles className="h-3 w-3" /> Academic Supervisor
              </span>
            </div>
          </div>
